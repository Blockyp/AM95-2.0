...
Uses a headless browser (Playwright) to load product pages, since some
retailers render price/stock with JavaScript after the initial page load
rather than including it in the raw HTML.
...
def fetch(url, retries=3, timeout=15):
    """Plain HTTP fetch — used for category pages and the Shopify JSON endpoint."""
...
def fetch_rendered(url, browser, retries=2, timeout=20000):
    """Load a page in a real headless browser and return the fully rendered
    HTML, so JavaScript-rendered price/stock content is present."""
    for attempt in range(retries):
        page = None
        try:
            ...
            print(f"  [warn] render attempt {attempt+1} failed for {url}: {e}")
            if page:
                try:
                    page.close()
                except Exception:
...
def parse_product_page(html, url=None):
    ...
    1. Shopify's public product JSON endpoint
    2. Open Graph / meta tags
    3. Twitter card data
    4. JSON-LD structured data
    5. Loose text scan of the (fully rendered) page"""
    price = None
    in_stock = None

    if url and "/products/" in url:
        base = url.split("?")[0].rstrip("/")
        shopify_json = fetch(base + ".json")
        if shopify_json:
            try:
                data = json.loads(shopify_json)
    ...
    if availability and in_stock is None:
        in_stock = availability.strip().lower() in ("instock", "in stock", "in_stock")

    if price is None:
        data1 = _meta(soup, "twitter:data1")
    ...
    if price is None or in_stock is None:
        ...
        if in_stock is None:
            out_of_stock = re.search(
                r"sold out|out of stock|unavailable|notify me when in stock|coming soon",
                text, re.IGNORECASE,
            )
...
def check_watchlist(watchlist, browser):
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            check_watchlist(watchlist, browser)
        finally:
            browser.close()
